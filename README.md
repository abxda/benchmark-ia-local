...
### Los cinco hallazgos que cambian cómo se elige un modelo
...
- **R es la debilidad universal.** Todos los modelos fallan en R de un turno (el campeón: 2/3). El fallo recurrente es el mismo en todos: fechas sin cargar `lubridate`. El bucle agéntico lo corrige solo — por eso el 6/6 solo aparece en modo agéntico.
...
| `llm_benchmark/results/` | Métricas crudas en JSON, una por corrida |
| `llm_benchmark/work/` | El código que cada modelo escribió (solo `.py` y `.R`; lo demás se regenera) |
